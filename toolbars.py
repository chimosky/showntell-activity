...
def get_mounts():
    ...
    for volume in volume_monitor.get_volumes():
        ...
        print "GETTING VOLUMES", volume.get_name(),\
...
class MakeToolBar(Gtk.Toolbar):
    ...
    def __init__(self, this_activity, deck):
        ...
        for i in range(0, len(ds_mounts), 1):
            print 'mount', i, ds_mounts[i]['uri'],\
                ds_mounts[i]['title'], ds_mounts[i]['id']
        ...
        # self.__journalbtn.connect(
        #    'clicked',
        #    self.chooseimage,
        #    ds_mounts[journal]['id'],
        #    DATASTORE)
    ...
    def new(self, widget):
        print 'New slideshow'
        # no effect if slideshow is already 'new', e.g. when ShowNTell is
        # opened directly not by read_file this needs to be changed to show
        # slideshow with html title slide
